...
function createBot(token, client, riskControl, logger) {
  ...
  bot.onText(/^\/price\s+(\S+)$/, async (msg, match) => {
    ...
    if (result.success && result.data) {
      const t = result.data;
      const last = t.last || t.price || 'N/A';
      const change = t.change || 'N/A';
      const changePct = t.change_rate ? `${(parseFloat(t.change_rate) * 100).toFixed(2)}%` : 'N/A';
      const msg2 = [
        `📊 *${contract} 行情*`,
        `最新价: \`${last}\``,
        `买一: \`${t.bid || 'N/A'}\`  卖一: \`${t.ask || 'N/A'}\``,
        `24h涨跌: \`${change} (${changePct})\``,
        `24h高: \`${t.high_24h || 'N/A'}\`  24h低: \`${t.low_24h || 'N/A'}\``,
        `24h成交量: \`${t.volume || 'N/A'}\``,
      ].join('\n');
      ...
      // 合约不存在时友好提示
      if (errMsg.includes('to parse a null string') || errMsg.includes('invalid') || errMsg.includes('404')) {
        safeSend(chatId, `❌ 合约 ${contract} 不存在，请检查合约名称是否正确\n示例: /price BTC_USDT`);
      } else {
        safeSend(chatId, `❌ 查询失败: ${errMsg}`);
      }
    }
  });
  ...
  bot.onText(/^\/buy\s+(\S+)\s+([\d.]+)$/, async (msg, match) => {
    ...
    if (isNaN(quantity) || quantity <= 0) {
      return safeSend(chatId, '⚠️ 数量必须为正数\n示例: /buy BTC_USDT 0.001');
    }
    // 风控
    ...
    if (result.success && result.data) {
      ...
      safeSend(chatId, `❌ 买入失败: ${result.error || result.data?.message || '未知错误'}`);
    }
  });
  ...
  bot.onText(/^\/sell\s+(\S+)\s+([\d.]+)$/, async (msg, match) => {
    ...
    if (result.success && result.data) {
      ...
      safeSend(chatId, `❌ 卖出失败: ${result.error || result.data?.message || '未知错误'}`);
    }
  });
